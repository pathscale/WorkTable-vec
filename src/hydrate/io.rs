...
//! table.write(&mut embedded_io_adapters::std::FromStd::new(file))?;
//!
...
impl<E> From<LoadError> for ReadError<E> {
...
impl<E: core::fmt::Debug + core::fmt::Display> core::error::Error for ReadError<E> {}
...
fn fill<R: Read>(
    ...
    page: &mut [u8],
    index: usize,
) -> Result<bool, ReadError<R::Error>> {
    let mut filled = 0;
    while filled < page.len() {
        match source.read(&mut page[filled..]).map_err(ReadError::Io)? {
...
fn read_rows<K, V, R: Read>(source: &mut R) -> Result<Vec<(K, V)>, ReadError<R::Error>>
where
    Vec<(K, V)>: Codec,
{
    ...
    if index == 0 {
        return Err(ReadError::Page(LoadError::NotWholePages { found: 0 }));
    }
    let expected = fingerprint::<Vec<(K, V)>>();
    match schema {
        Some(found) if found != expected => {
            Err(ReadError::Page(LoadError::ForeignRows { found, expected }))
        }
        _ => Ok(rows),
    }
}
    ...
    (K, V): Clone,
{
    ...
    pub fn write<W: Write>(&self, sink: &mut W) -> Result<(), W::Error> {
    ...
    ///
    /// Whatever the writer reports.
    pub fn append<W: Write>(&self, sink: &mut W, first: usize) -> Result<(), W::Error> {
    ...
    pub fn read<R: Read>(source: &mut R) -> Result<Self, ReadError<R::Error>> {
        Ok(Self {
